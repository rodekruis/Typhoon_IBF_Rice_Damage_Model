...
#%>% passes the left hand side of the operator to the right hand side: x %>% f means f(x)
#mutate adds new variables and preserves existing ones
#$ revers to a specific column in a dataframe
#Input file is a file of the hurricane track that contains at least several columns: YYYMMDDHH, LAT, LON, VMAX, STORMNAME
...
create_full_track <- function(hurr_track, tint = 3){ 
  ...
  hurr_track <- hurr_track %>%
    dplyr::mutate(
      ...
      date                           = lubridate::ymd_h(YYYYMMDDHH),
  ...
  #interp_df<- 34 # 108 --30
  #Date in increasing steps of 3 hours
...
##While the Newton-Raphson method can sometimes perform poorly in finding global maxima, in this case the function for which we are trying ##to find the root is well-behaved. Across tropical storms from 1988 to 2015, the method never failed to converge, and identified roots ##were consistent across storms (typically roots are for ÃÂ¾ of 0.6--0.65). We also tested using the optim function in the stats R package ##and found similar estimated roots but slower convergence times than when using the Newton-Raphson method.

##Now an equation from the Willoughby et al. 2006 paper can be used to calculate R1 (Willoughby, Darling, and Rahn 2006):

#R1?=? Rmax? ? ??(R2? ? ? R1)

#For this function, the package code assumes that R2Ã¢ÂÂÃ¢ÂÂÃ¢ÂÂR1 (the width of the transition region) is 25 kilometers when Rmax is larger than 20 #kilometers and 15 kilometers otherwise.
...
add_wind_radii <-function(full_track = create_full_track()){
  ...
  with_wind_radii <- full_track %>%
    dplyr::mutate_(tcspd = ~ calc_forward_speed(tclat, tclon, date,
                   ...
                   over_land = ~ mapply(check_over_land, tclat, tclon), #applies function over each argument
...
#THIS IS THE FINAL EQUATION THAT SHOULD BE RUN AND USES ALL PREVIOUS EQUATIONS
get_grid_winds<- function(hurr_track , grid_df ,tint = 3,gust_duration_cut = gust_duration_cut,sust_duration_cut = sust_duration_cut){
...
summarize_grid_wind <- function(grid_wind, tint = 3, gust_duration_cut = gust_duration_cut,
                                sust_duration_cut = sust_duration_cut){
  ...
  grid_wind_summary <- grid_wind %>%
    ...
    dplyr::summarize_(vmax_gust = ~ max(gustspeed, na.rm = TRUE),
                      ...
                      sust_dur = ~ 60 * sum(windspeed > sust_duration_cut,
                                            na.rm = TRUE),
                      dist_track_dur = ~ 60 * sum(cdist<dist_track_cut, na.rm=TRUE),
                      vavg_gust = ~ mean(gustspeed, na.rm=TRUE),
                      vavg_sust = ~ mean(windspeed, na.rm=TRUE)
    ) %>%
    dplyr::mutate_(gust_dur = ~ gust_dur * tint,
                   sust_dur = ~ sust_dur * tint,
                   dist_track_dur = ~ dist_track_dur * tint)
...
calc_and_summarize_grid_wind <- function(grid_point,
                                         ...
                                         tint = 3, gust_duration_cut = gust_duration_cut,
                                         sust_duration_cut = sust_duration_cut){
...
setwd('C:/Users/Marieke/GitHub/Rice_Field_Damage_Philippines/app/data/')

#Creating list of typhoons for which calculations should be done (in CAPS)
typhoons_list = list('AERE2011', 'HAGUPIT2014', 'HAIKUI2017', 'HAIYAN2013', 'JANGMI2014', 'KAI-TAK2017', 'KALMAEGI2014', 'KAMMURI2019', 'MEARI2011', 'MEKKHALA2015', 'MELOR2015', 'MIRINAE2009', 'NESAT2011', 'NOCK-TEN2011', 'NOCK-TEN2016', 'PARMA2009', 'UTOR2013', 'WASHI2011')

typhoons_list = list('KETSANA2009', 'KOMPASU2010', 'NOCK-TEN2011', 'NANMADOL2011', 'NALGAE2011', 'UTOR2013','MANGKHUT2018', 'YUTU2018', 'NAKRI2019', 'DANAS2019', 'KAMMURI2019', 'VAMCO2020', 'HAIMA2016', 'KOPPU2015', 'GONI2015', 'MOLAVE2020', 'USAGI2013', 'NARI2013', 'KAI-TAK2012', 'KROSA2013', 'MATMO2014', 'FUNG-WONG2014','MELOR2015', 'SARIKA2016', 'PAKHAR2017', 'KHANUN2017', 'RAMMASUN2014', 'KALMAEGI2014', 'MUJIGAE2015', 'MALIKSI2018', 'VONGFONG2020', 'SAUDEL2020', 'GONI2020', 'NOCK-TEN2016', 'DURIAN2006', 'NESAT2011', 'BOPHA2012', 'TRAMI2013', 'HAIYAN2013', 'TOKAGE2016', 'LINGLING2014', 'ATSANI2020', 'TEMBIN2017', 'WASHI2011', 'LINFA2015', 'HAGUPIT2014', 'JANGMI2014', 'PHANFONE2019', 'KAI-TAK2017', 'AERE2011', 'HAIKUI2017', 'MEARI2011', 'MEKKHALA2015', 'MIRINAE2009', 'PARMA2009', 'KROVANH2020')


#Reading in the entire best track data sheet
best_track_data <- read.csv("source_data/wp_best_track_2000-2021.csv")

#Reading in the landmask sheet
landmask <- readr::read.csv("Wind/stormwindmodel/input_files/landseamask_ph1.csv",col_names = c("longitude", "latitude", "land")) %>%
  dplyr::mutate(land = factor(land, levels = c(1, 0), labels = c("land", "water")))

#Reading in the administrative boundaries
grid_df <- read.csv(file = "Wind/stormwindmodel/input_files/grid_points_admin3.csv")

#Names of columns to select from dataframe (can be altered when using different agency)
STORMNAME = "STORMNAME"
DATE = "ISO_TIME"
LAT = "LAT"
LON = "LON"
...
wind_duration_cut_list = list(20)
dist_track_cut_list = list(20)
dist_track_cut = 20


for (typhoon  in typhoons_list){
  print(typhoon)
  
  #Get the best track data only for the specific typhoon & select the needed columns
  df <- subset(best_track_data, STORMNAME == typhoon)  
  hurr_track <- df %>% select(STORMNAME, DATE, LAT, LON, VMAX)
  colnames(hurr_track) <- c('STORMNAME', 'DATE', 'LAT', 'LON', 'VMAX')
  
  hurr_track$YYYYMMDDHH = substr(hurr_track$DATE, 1, 13)
  
  output <- get_grid_winds(hurr_track, grid_df, tint = tint, gust_duration_cut = wind_duration_cut_list[[1]], sust_duration_cut = wind_duration_cut_list[[1]])
  
  for (cut in wind_duration_cut_list){
    ad_output <- get_grid_winds(hurr_track, grid_df, tint = tint, gust_duration_cut = cut, sust_duration_cut = cut)
    output[paste('gust_', as.character(cut), sep="")] <- ad_output$gust_dur
    output[paste('sust_', as.character(cut), sep="")] <- ad_output$sust_dur
    }
  
  
  colnames(output)[1] <- 'gridid'
  
  typhoon_lower = tolower(typhoon)
  
  file_name = paste("Wind/stormwindmodel/output_files/", typhoon_lower, "_windgrid_output.csv", sep="")
