...
def cumulative_rainfall(
    admin_geometry, start_date, end_date, download_path, type_imerg, tf, mw
):
    ...
    if file_list:
        ...
        sum_raster_list = rolling_window_values(raster_list, tf, mw)

        sum_rainfall_list = []
        for i in range(len(sum_raster_list)):
            print(
                "Sum rainfall in area calculation, iteration ", i, "For typhooon ", key
            )
            ...
            sum_rainfall_list.append(sum_rainfall)

        n_areas = len(sum_rainfall_list[0])
        maximum = []
        for i in range(n_areas):
            m = max([x[i]["mean"] for x in sum_rainfall_list])
            maximum.append(m)

        # Give output in cl per timeframe (eg. 6h)
        # tf/2 because tf is given hhr and quantity is cl/h
        maximum_rainfall = [i / 10 * (tf / 2) for i in maximum]
...
def process_tyhoon_data(typhoon_to_process, typhoon_name, p_code, tf, mw):
    ...
    output_matrix_csv_name = typhoon_name + "_" + str(tf) + "_HHR_matrix.csv"
    ...
    gpm_path = os.path.join(workspace, "GPM_HHR")
    output_matrix_csv_file = os.path.join(workspace, output_matrix_csv_name)

    output_columns = ["area_km2", "Mun_Code", "Rainfall", "geometry"]
    ...
    if not imerg_type == "trmm":
        output_gdf["Rainfall"] = cumulative_rainfall(
            admin_geometry_wgs84, start_date, end_date, gpm_path, imerg_type, tf, mw
        )
...
##########################
"""
Fill in the input variables needed below
"""
...
p_code = "ADM3_PCODE"
# Half hourly time fram (eg. 12 = 6h)
time_frame = int(48)
# Window for calculating the maximum (eg. 2 = 1h)
moving_window = int(2)
# Set to true if you want to delete all the data previously outputted and start from scratch
# eg. if the dates have changed and you want to download new GPM data
...
admin_file_name = (
    "PHL_administrative_boundaries/phl_admbnda_adm3_psa_namria_20200529.shp"
)
...
for key in typhoons_dict:
    ...
    dir_temp = os.path.abspath(
        os.path.join(current_path, "output_data", key, "GPM_HHR")
    )
    print(dir_temp)
    ...
    if delete_folders == True:
        ...
        try:
            ...
            print(key, " GPM_HHR folder removed")
        except:
            print("No ", key, " GPM_HHR folder present")

    process_tyhoon_data(typhoons_dict[key], key, p_code, time_frame, moving_window)
