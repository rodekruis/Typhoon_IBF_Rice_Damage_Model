# This processing pipeline is for gettign the before, during, after rainfall variables WITH the per municipality enter and exit times
...
# Processing the data
pre_hours = 72  # hours before entry
post_hours = 48  # hours after exit
...
for sid, group in grouped_entry_exit_data:
    ...
    for index, row in group.iterrows():
        ...
        entry_time = datetime.datetime.strptime(row['entry_date'], '%Y-%m-%d %H:%M:%S')
        exit_time = datetime.datetime.strptime(row['exit_date'], '%Y-%m-%d %H:%M:%S')
        
        # Calculate the pre and post periods
        pre_start = entry_time - datetime.timedelta(hours=pre_hours)
        post_end = exit_time + datetime.timedelta(hours=post_hours)
        ...
        pre_rainfall = df_mun_rainfall[(df_mun_rainfall.index >= pre_start) & (df_mun_rainfall.index < entry_time)]
        pre_max_6h = pre_rainfall.rolling(window=window_size_6h*2).sum().max().values[0]
        pre_max_24h = pre_rainfall.rolling(window=window_size_24h*2).sum().max().values[0]

        # During Typhoon Period Calculation
        during_rainfall = df_mun_rainfall[(df_mun_rainfall.index >= entry_time) & (df_mun_rainfall.index <= exit_time)]
        during_total_rainfall = during_rainfall.sum().values[0]
        during_max_rainfall = during_rainfall.max().values[0]
        during_mean_rainfall = during_rainfall.mean().values[0]
        during_max_1h_intensity = during_rainfall.rolling(window=2).sum().max().values[0]  # max 1-hour intensity
        during_max_3h_intensity = during_rainfall.rolling(window=6).sum().max().values[0]  # max 3-hour intensity
        during_max_6h_intensity = during_rainfall.rolling(window=12).sum().max().values[0]  # max 6-hour intensity

        # Post-Typhoon Period Calculation
        post_rainfall = df_mun_rainfall[(df_mun_rainfall.index > exit_time) & (df_mun_rainfall.index <= post_end)]
        post_max_6h = post_rainfall.rolling(window=window_size_6h*2).sum().max().values[0]
        post_max_24h = post_rainfall.rolling(window=window_size_24h*2).sum().max().values[0]

        # Collect the results in a DataFrame
        result = pd.DataFrame([{
            'typhoon': typhoon_name,
            'mun_code': mun_code,
            'pre_max_6h': pre_max_6h,
            'pre_max_24h': pre_max_24h,
            'during_total_rainfall': during_total_rainfall,
            'during_max_rainfall': during_max_rainfall,
            'during_mean_rainfall': during_mean_rainfall,
            'during_max_1h_intensity': during_max_1h_intensity,
            'during_max_3h_intensity': during_max_3h_intensity,
            'during_max_6h_intensity': during_max_6h_intensity,
            'post_max_6h': post_max_6h,
            'post_max_24h': post_max_24h
        }])
...
file_path = "IBF_typhoon_model/data/rainfall_data/rainfall_variables.csv"
