...
#Loading in the predicted values from pickle
# infile = open('app\\outputs\\regressions\\svr\\svr_df_predicted_01', 'rb')
# df_predicted = pickle.load(infile)
# infile.close()

#Loading in the predicted values from excel sheet
file_name = 'app\\outputs\\regressions\\tobit_regression\\tobit_prediction_01.xlsx'
path = os.path.join(cwd, file_name)
df_predicted = pd.read_excel(path)

#For visualizing multiclass classification
print(df_predicted['kammuri2019'])
df_predicted['kammuri2019'] = [0 if i < 0.4 else 1 if (i<0.7) & (i>0.4) else 2 for i in df_predicted['kammuri2019']]
print(df_predicted['kammuri2019'])

# columns = df_predicted.columns
# # print(df_predicted[columns[1:]].head())
# df_predicted.loc[df_predicted[columns[1:]] < 0.3] = 0

# df_predicted['perc_loss'] = [0 if i < 0.3 else 1 if (i<0.5 & i>0.3) else 2 for i in df_predicted['perc_loss']]
...
for i in range(len(df_actual)):
    for typh in typhoons:
        value = df_source['perc_loss'][(df_source['municipality_codes']==df_actual['municipality_code'][i]) & (df_source['typhoons']==typh)].values[0]
...
def update_graph(val_chosen):
    fig = px.choropleth_mapbox(df_actual, geojson=map_dict[val_chosen], color=val_chosen,
                           ...
                           mapbox_style="carto-positron", zoom=6, width=900, height=700, color_continuous_scale='amp', range_color=[0,1], color_discrete_map={'black', 1})
    return fig
